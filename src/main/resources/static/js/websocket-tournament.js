(() => {
        ...
        //game status tracker
        const playerStatusDiv = document.querySelector('#player-div');
        const loserStatusDiv =  document.querySelector('#loser-div');
        //chat footer
        const chatMessages = document.querySelector('#chat-messages');
        const chatSendBtn = document.querySelector('#chat-submit-btn');
        ...
        const Socket = {
            ...
            async onMessageReceived(payload) {
                ...
                if (message.messageType === 'JOIN') {
                    ...
                    if(gameController.statusHeader = false){
                        Render.statusHeader();
                        gameController.statusHeader = true;
                    }
...
`;
           ...
           <div class="container results-container">
            ...
            statusHeader(){
                gameController.activePlayers.forEach((player)=>{
                    playerStatusDiv.innerHTML += `
                <div class="player-status-label">${player}</div>`
                })
...
//event to remove a user that left the page and reload the page for all other users
        window.onbeforeunload = (event) => {
            ...
            Socket.sendMessage(message);
            return "Are you sure you want to leave the tournament?"
        };
